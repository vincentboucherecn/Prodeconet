...
file.copy("comparisons0.R",paste(scripttime,".txt",sep=""))
...
simul <- function(nnet){
  ...
  saveRDS(out,paste("data/simu0_",nnet,"_",parprod[[2]][1],".Rda",sep=""))
  ct <- lapply(1:nrow(out),function(i) catches(out[i,2:31],parprod))
  ct <- do.call(rbind,ct)
  saveRDS(ct,paste("data/catch0_",nnet,"_",parprod[[2]][1],".Rda",sep=""))
...
alist <- seq(from=0.01,to=0.105, by=0.005)/4
